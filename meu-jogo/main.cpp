...
enum GameState { STATE_MENU, STATE_GAME, STATE_PAUSE };
...
int main() {
    ...
    // --- CONFIGURAÇAO DE TELA CHEIA ---
    al_set_new_display_flags(ALLEGRO_FULLSCREEN_WINDOW);
    ALLEGRO_DISPLAY* display = al_create_display(SCREENWIDTH, SCREENHEIGHT);
    al_set_window_title(display, "Back 2 Start");

    ALLEGRO_BITMAP* icon = al_load_bitmap("assets/icon/logo.png");
    if (icon) {
        al_set_display_icon(display, icon);
    }
    else {
        printf("Aviso: Icone nao encontrado em assets/icon.png\n");
    }

    // --- CALCULO DE ESCALA (ZOOM) ---
    // Pega o tamanho real do monitor do usuario
    int monitorW = al_get_display_width(display);
    int monitorH = al_get_display_height(display);

    // Calcula o fator de zoom (mantendo proporcao)
    float sx = monitorW / (float)SCREENWIDTH;
    float sy = monitorH / (float)SCREENHEIGHT;
    float scale = (sx < sy ? sx : sy);

    // Calcula as barras pretas para centralizar
    float scaleW = SCREENWIDTH * scale;
    float scaleH = SCREENHEIGHT * scale;
    float scaleX = (monitorW - scaleW) / 2;
    float scaleY = (monitorH - scaleH) / 2;
    ...
    int btnW = 300;
    int btnH = 50;
    int centerX = SCREENWIDTH / 2 - (btnW / 2); // Centraliza baseado no tamanho do jogo
    ...
    while (gameRunning) {
                    ...
                    while (enet_host_service(netHost, &netEvent, 0) > 0) {
                        ...
                        case ENET_EVENT_TYPE_RECEIVE:
                            if (netEvent.packet->dataLength >= sizeof(int)) {
                                ...
                                if (!isServer && type == PACKET_WELCOME) {
                                    ...
                                    if (type == PACKET_STATE) {
                                        ...
                                        if (pkt->id >= 0 && pkt->id < players.size()) {
                                            players[pkt->id]->setNetworkState(pkt->x, pkt->y, pkt->current_frame_y, pkt->isMoving);
                                        }
                                    }
        ...
        // O Allegro manda a coordenada REAL do mouse na tela.
        // Precisamos converter para a coordenada VIRTUAL do jogo (sem o zoom e o offset)
        else if (ev.type == ALLEGRO_EVENT_MOUSE_AXES) {
            gameMouseX = (ev.mouse.x - scaleX) / scale;
            gameMouseY = (ev.mouse.y - scaleY) / scale;
        }
        else if (ev.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN) {
            // Atualiza tambem no click para garantir precisao
            gameMouseX = (ev.mouse.x - scaleX) / scale;
            gameMouseY = (ev.mouse.y - scaleY) / scale;
        }
        ...
        if (currentState == STATE_MENU) {
            if (ev.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN) {
                // Usamos gameMouseX ao inves de ev.mouse.x
                if (btnHost.isOver(gameMouseX, gameMouseY)) {
                    if (startHost()) {
                        baseMap.loadMapFromJson("assets/maps/level2.json");
                        players.clear();
                ...
                else if (btnExit.isOver(gameMouseX, gameMouseY)) {
                    gameRunning = false;
                }
            }
        ...
        if (redraw && al_is_event_queue_empty(queue)) {
            redraw = false;
            // Limpa a tela (preto nas bordas)
            al_clear_to_color(al_map_rgb(0, 0, 0));

            // Tudo desenhado a partir daqui vai sofrer o ZOOM e CENTRALIZACAO automaticamente

            if (currentState == STATE_MENU) {
                al_draw_text(font, al_map_rgb(50, 50, 50), SCREENWIDTH / 2 + 4, 54, ALLEGRO_ALIGN_CENTRE, "BACK 2 START");
