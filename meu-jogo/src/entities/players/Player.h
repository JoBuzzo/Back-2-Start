...
public:
    ...
    Player() : sprite(nullptr), w(32), h(32), posX(0), posY(0),
               ...
               terrainFactor(1.0f), speed(2.0f), debugMode(true)
    {
    }
    ...
    virtual void keyDOWN(int keycode)
    {
        ...
        case ALLEGRO_KEY_SPACE: jump(); break;
        }
    }
    ...
    virtual void updateDirection()
    {
        ...
        else if (keys[A]) current_frame_y = 1;
        else if (keys[D]) current_frame_y = 2;
        else if (keys[W]) current_frame_y = 3;
    }
    ...
    virtual void move()
    {
        ...
        if (isRunning) currentSpeed *= 1.5f;
        if (isJumping && isRunning) currentSpeed *= 1.2f;

        int dx = 0, dy = 0;
        if (keys[W]) dy -= (int)currentSpeed;
        if (keys[S]) dy += (int)currentSpeed;
        if (keys[A]) dx -= (int)currentSpeed;
        if (keys[D]) dx += (int)currentSpeed;
    ...
    bool handleBorderCollision()
    {
        ...
        if (hx < 0) { posX += (0 - hx); col = true; }
        else if (hx + hw > mapW) { posX -= (hx + hw - mapW); col = true; }

        if (hy < 0) { posY += (0 - hy); col = true; }
        else if (hy + hh > mapH) { posY -= (hy + hh - mapH); col = true; }
    ...
    void drawHitbox() {
        int hx, hy, hw, hh;
        getHitbox(hx, hy, hw, hh); 
        float drawY = hy - (int)z; 
        al_draw_rectangle(hx, drawY, hx + hw, drawY + hh, al_map_rgb(255, 0, 0), 1);
    }
