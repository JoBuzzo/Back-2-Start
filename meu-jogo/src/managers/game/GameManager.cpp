    ...
    net.update(
        ...
        [&](ENetEvent &event) {                // Receive
            ...
            { // SERVIDOR
                ...
                {
                    if (pkt->isDown)
                        players[pkt->playerId]->keyDOWN(pkt->keycode);
                    else
                        players[pkt->playerId]->keyUP(pkt->keycode);
...
void GameManager::updateGameLogic()
{
    ...
    for (size_t i = 0; i < (size_t)activePlayers; i++)
    {
        ...
        if (props.deadly && players[i]->z <= 2.0f)
        {
...
void GameManager::draw()
{
    ...
    else if (currentState == STATE_GAME || currentState == STATE_PAUSE)
    {
        if (level.isLoaded)
        {
            ...
            int clipX = (int)scaleX;
            int clipY = (int)scaleY;
            int clipW = (int)(WMAP * BLOCKSIZE * scale);
            int clipH = (int)(HMAP * BLOCKSIZE * scale);
            ...
            level.drawWeather();
            if (fontSmall)
                al_draw_text(fontSmall, al_map_rgb(255, 255, 255), SCREENWIDTH - 10, 10, ALLEGRO_ALIGN_RIGHT, level.title.c_str());
        }
        else
...
void GameManager::handleInput(ALLEGRO_EVENT &ev)
{
    ...
    else if (currentState == STATE_GAME)
    {
        if (ev.type == ALLEGRO_EVENT_KEY_DOWN)
        {
            ...
            else
            {
                ...
                {
                    InputPacket pkt;
                    pkt.playerId = net.getMyId();
                    pkt.keycode = ev.keyboard.keycode;
                    pkt.isDown = isDown;
    ...
    else if (currentState == STATE_PAUSE)
    {
        if (ev.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN)
        {
            if (btnResume.isOver(gameMouseX, gameMouseY))
                currentState = STATE_GAME;
